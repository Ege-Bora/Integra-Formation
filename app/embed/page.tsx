...
export default function EmbedFormPage() {
  ...
  const getVisibleSteps = () => {
    ...
    steps.push({ id: 19, key: 'indicative_cost_calculation' });
    steps.push({ id: 20, key: 'document_upload' });
  ...
  const handleNext = async () => {
    ...
    // Special handling for step 19 - if "Hayır" is selected, end the form
    if (stepId === 19) {
      const indicativeValue = watchedValues.indicative_cost_calculation;
      if (indicativeValue === 'Hayır') {
        // Submit the form and show success
        handleSubmit(onSubmit)();
        return;
      }
    }
    
    // Skip validation for optional financial questions (4-7)
    ...
    const isValid = await trigger(currentStepData.key as any);
    if (isValid) {
      if (currentStep < totalSteps) {
        setCurrentStep(currentStep + 1);
      }
  ...
  const handleKeyDown = (e: React.KeyboardEvent) => {
    if (e.key === 'Enter' && !e.shiftKey) {
      e.preventDefault();
      if (currentStep === totalSteps) {
        console.log('Enter pressed on final step, calling handleSubmit');
        handleSubmit(onSubmit)();
      } else {
        handleNext();
      }
  ...
  const handleFormSubmit = async (e: React.FormEvent) => {
    ...
    if (currentStep === totalSteps) {
      console.log('On final step, calling handleSubmit(onSubmit)');
      const result = await handleSubmit(onSubmit)();
      console.log('handleSubmit result:', result);
    } else {
      console.log('Not on final step, calling handleNext');
      handleNext();
    }
  };
  ...
  console.log('Current state - showFinalCostEstimation:', showFinalCostEstimation, 'showSuccess:', showSuccess);
  ...
  const renderStepContent = () => {
    switch (stepId) {
      ...
      case 19:
        return (
          <RadioGroup
            name="indicative_cost_calculation"
            options={["Evet", "Hayır"]}
            value={watchedValues.indicative_cost_calculation}
            onChange={(value) => setValue('indicative_cost_calculation', value as any)}
            register={register}
            error={errors.indicative_cost_calculation}
          />
        );
        ...
        return (
          <div className="space-y-4">
            <p className="text-gray-600 mb-4">
              Doküman yükleme alanı - yakında eklenecek
            </p>
            <div className="p-8 border-2 border-dashed border-gray-300 rounded-lg text-center">
              <p className="text-gray-500">Doküman yükleme özelliği geliştiriliyor...</p>
            </div>
          </div>
        );
  ...
  const getStepTitle = () => {
    switch (stepId) {
      ...
      case 4: return "Yıllık gelir tutarınızı seçiniz (USD). (İsteğe bağlı)";
      case 5: return "Yıllık ithalat tutarınızı seçin (USD) (İsteğe bağlı)";
      case 6: return "Yıllık ihracat tutarınızı seçin (USD) (İsteğe bağlı)";
      case 7: return "Yıllık kârlılık tutarınızı seçin (USD) (İsteğe bağlı)";
      ...
      case 19: return "Endikatif Maliyet Hesabı Görmek İster Misiniz?";
      case 20: return "Doküman Yükleme";
      default: return "";
    }
  };
